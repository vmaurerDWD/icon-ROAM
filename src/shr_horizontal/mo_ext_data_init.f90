...
#ifdef COUP_OASIS3MCT
          IF (.NOT. do_patch_land_sea_mask) THEN
            CALL finish(routine, 'cell_sea_land_mask not in extpar, but mandatory for NEMO coupling!')
          END IF
#endif
...
#ifdef COUP_OASIS3MCT
    INTEGER :: i_count_nemo            ! counter for active NEMO-coupling points
#endif
    INTEGER :: rl_start, rl_end
    INTEGER :: i_startblk, i_endblk    !> blocks
    INTEGER :: i_startidx, i_endidx    !< slices
...
#ifdef COUP_OASIS3MCT
         i_count_nemo                  = 0   ! counter for active NEMO-coupling points
#endif
...
#ifdef COUP_OASIS3MCT
           !
           ! searching for active points for NEMO coupling
           !
           IF ( (ext_data(jg)%atm%lsm_ctr_c(jc,jb) <= (1 - frsea_thrhld)) .AND. &
           &    (ext_data(jg)%atm%lsm_ctr_c(jc,jb) >= 0.) ) THEN
             i_count_nemo = i_count_nemo + 1
             ext_data(jg)%atm%list_sea_nemo%idx(i_count_nemo,jb) = jc  ! write index of sea-points
             ext_data(jg)%atm%list_sea_nemo%ncount(jb) = i_count_nemo
           ENDIF
#endif
         END DO ! jc
...
!$OMP SINGLE
       ...
       !
#ifdef COUP_OASIS3MCT
       npoints = ext_data(jg)%atm%list_sea_nemo %get_sum_global(i_startblk,i_endblk)
       WRITE(message_text,'(a,i3,a,i10)') 'Number of active NEMO-coupling points in domain', &
        &                                  jg,':', npoints
       CALL message('', TRIM(message_text))
#endif
       !
...
#ifdef COUP_OASIS3MCT
    REAL(wp) :: fr_land_old, fr_lake_old, new_over_old
#endif
...
!$OMP DO PRIVATE(jb,jc,i_startidx,i_endidx,lat,lon)
    DO jb = i_startblk, i_endblk
      ...
      DO jc = i_startidx,i_endidx
        ...
        ! land-sea-mask switched by ocean:
        !-1: outside of considered domain (necessary for ocean points in ICON not covered by NEMO)
        ! 0: no change
        ! 1: new land
        ! 2: new ocean
        ! 3: modified fr_land, set fr_lake = 0 (coastal point)
        ! 4: modified fr_land and fr_lake
...
#ifdef COUP_OASIS3MCT
        ! necessary for regional ocean coupling as the extents of the
        ! A/O limited-area domains may not fully overlap
        IF (ext_data%atm%lsm_ctr_c(jc,jb) == -1.) THEN
          ext_data%atm%lsm_switch   (jc,jb) = -1
        ELSE

          ! fractional lsm from NEMO (uncommon grid), lsm_ctr_c is float!
          ! no decision tree, always overwrite fr_land
          fr_land_old = ext_data%atm%fr_land(jc,jb)
          fr_lake_old = ext_data%atm%fr_lake(jc,jb)

          ! ocean or coastal point according to NEMO LSM; set ocean and land only, no lake
          IF (ext_data%atm%lsm_ctr_c(jc,jb) <= (1 - frsea_thrhld)) THEN
            !  new (fractional) ocean point?
            IF ( (fr_land_old + fr_lake_old) > (1 - frsea_thrhld) ) THEN
              ! ** case 2 **
            ...
            ! adapt land/ocean fraction, but only if not pure ocean in new or old (or both) LSM
            ! (nothing to do if already pure ocean in both)
            ELSE IF (fr_land_old > frlnd_thrhld .OR. fr_lake_old > frlake_thrhld .OR. &
             &       ext_data%atm%lsm_ctr_c(jc,jb) > frlnd_thrhld) THEN
              ! ** case 3 **
          ...
          ! new (fractional) land in ICON? can occur for both coastal or land-only points!
          ! (case 1 is default; 6 for Dead and Caspian Sea):
          IF (fr_land_old <= frlnd_thrhld .AND. ext_data%atm%fr_land(jc,jb) > frlnd_thrhld) THEN
              ext_data%atm%lsm_switch (jc,jb)   = 1
              ext_data%atm%soiltyp    (jc,jb)   = 4           ! soil type to sandy loam (sfc_terra_data)
              ext_data%atm%lu_class_fraction(jc,jb,:)                       = 0.0_wp
              ext_data%atm%lu_class_fraction(jc,jb,ext_data%atm%i_lc_grass) = 1.0_wp  ! grass-land
              ...
              ! (treated as ocean in ICON-NWP, but land in ICON-O)
              ...
              IF ( ( (lon > 35._wp).AND.(lon < 36._wp) .AND. (lat > 31._wp).AND.(lat < 32._wp) ) .OR. &
                 ...
                 & ( (lon > 46._wp).AND.(lon < 55._wp) .AND. (lat > 36._wp).AND.(lat < 48._wp) ) ) THEN
                  ...
                  ext_data%atm%fr_lake    (jc,jb)   = 1._wp - fr_land_old
