...
!$OMP END PARALLEL
  ...
  !!
  SUBROUTINE init_coriolis( lcorio, lplane, ldeepatmo, patch )

    LOGICAL,INTENT(in) :: lcorio, lplane, ldeepatmo
    ...
    LOGICAL :: is_plane, is_traditional
    REAL(wp) :: zlat

    REAL(wp) :: primal_mer_comp, dual_mer_comp, magnitude
    REAL(wp) :: coriolis_param_hor
    ...
    IF (lcorio .AND. .NOT. is_plane) THEN
      ...
      IF (ldeepatmo) THEN
        ...
        DO jb = 1, nblks_e
          ...
          ELSE
            nlen = npromz_e
            patch%edges%fn_e(nlen+1:nproma,jb) = 0.0_wp
            patch%edges%ft_e(nlen+1:nproma,jb) = 0.0_wp
          ENDIF
          DO je = 1, nlen
            ...
            IF (magnitude > 0._wp) THEN
              ...
              primal_mer_comp = patch%edges%primal_normal(je,jb)%v2 / magnitude
            ELSE
              primal_mer_comp = 0._wp
            ENDIF

            ! the same procedure for the 'dual_normal' T
            magnitude = SQRT(patch%edges%dual_normal(je,jb)%v1**2 + patch%edges%dual_normal(je,jb)%v2**2)
            IF (magnitude > 0._wp) THEN
              dual_mer_comp = patch%edges%dual_normal(je,jb)%v2 / magnitude
            ELSE
              dual_mer_comp = 0._wp
            ENDIF
