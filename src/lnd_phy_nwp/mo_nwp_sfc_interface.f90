...
MODULE mo_nwp_sfc_interface
  ...
  USE mo_lnd_nwp_config,      ONLY: nlev_soil, nlev_snow, ibot_w_so, ntiles_total,    &
    ...
    &                               lprog_albsi, itype_trvg, lterra_urb, hice_min,    &
...
#ifdef COUP_OASIS3MCT
    LOGICAL, OPTIONAL,           INTENT(in)   :: lcpl_hice     !< usage of h_ice for NEMO coupling
#endif
    LOGICAL :: lzacc
...
#ifdef COUP_OASIS3MCT
        &             lnd_prog_now, lnd_prog_new, ext_data, lnd_diag, tcall_sfc_jg, lcpl_hice, lacc=lzacc)
#else
...
!$OMP END PARALLEL
  ...
  SUBROUTINE nwp_seaice (p_patch, p_diag, prm_diag, p_prog_wtr_now,  &
...
#ifdef COUP_OASIS3MCT
    &                    ext_data, p_lnd_diag, dtime, lcpl_hice, lacc)
#else
    ...
    REAL(wp),                    INTENT(in)   :: dtime          !< time interval for 
                                                                !< surface
#ifdef COUP_OASIS3MCT
    LOGICAL,                     INTENT(in)   :: lcpl_hice      !< switch for coupling of hice+tice
#endif
...
!$NEC ivdep
...
#ifdef COUP_OASIS3MCT
      ! in the NEMO area, overwrite alb_si (and optionally h_ice and t_ice) with the old values,
      ! i.e. overwrite output of seaice_timestep_nwp;
      ! with that, in the NEMO area these fields are only updated at the coupling time step (1 h currently)!!
      ...
      IF (lcpl_hice) THEN
        DO ic = 1, ext_data%atm%list_sea_nemo%ncount(jb)
          jc = ext_data%atm%list_sea_nemo%idx(ic,jb)
          IF (p_prog_wtr_now%h_ice(jc,jb) > hice_min) THEN
            p_prog_wtr_new%h_ice(jc,jb) = p_prog_wtr_now%h_ice(jc,jb)
            p_prog_wtr_new%t_ice(jc,jb) = p_prog_wtr_now%t_ice(jc,jb)

            lnd_prog_new%t_g_t(jc,jb,isub_seaice) = p_prog_wtr_now%t_ice(jc,jb)
            ! surface saturation specific humidity (uses saturation water vapor pressure over ice)
            p_lnd_diag%qv_s_t(jc,jb,isub_seaice)  = spec_humi(sat_pres_ice(p_prog_wtr_now%t_ice(jc,jb)), &
              &                                     p_diag%pres_sfc(jc,jb) )
          ENDIF
        ENDDO
      ENDIF
